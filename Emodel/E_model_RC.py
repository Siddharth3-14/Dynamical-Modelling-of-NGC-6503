...
def Disc(R,R_d = 0.968):
    temp = 4*790.678*np.pi*0.24*np.exp(-R/R_d)*R
    return temp
...
with total_model:
    ...
    total_rot =  disk_rot + halo_rot 

    #likelihood
    Y_obs = pm.Normal('Y_obs', mu = total_rot, sigma = sigma, observed = V_obs2)
    step = pm.Metropolis()
    trace = pm.sample(draws = 1500000, step = step, tune = 1000, cores = 2)



print(pm.summary(trace))
parameter_mean = pm.summary(trace)["mean"]
model_pred = (parameter_mean[4]*M_R_disk + (parameter_mean[3]*(Radial_distance**(parameter_mean[1] - 1)))/((parameter_mean[2]**parameter_mean[1])*(1 + ((Radial_distance/parameter_mean[2])**(parameter_mean[1]-1)) )))**0.5
disk_pred = (parameter_mean[4]*M_R_disk)**0.5
